...
//        }
    ...
    public static String duration( long duration ) {
        ...
        seconds = duration;
        return ((0 < hours && hours > 9) ? Long.toString( hours ) : "0" + hours) +
                ":" +
                ((0 < minutes && minutes > 9) ? Long.toString( minutes ) : "0" + minutes) +
                ":" +
                ((0 < seconds && seconds > 9) ? Long.toString( seconds ) : "0" + seconds) +
                "s";
    }
    ...
    public static long min( long num1, long num2, long num3 ) {
        return min( min( num1, num2 ), min( num2, num3 ) );
    }
    ...
    public static ByteArrayOutputStream exchangeObjectToXMLFile( final OpxiCMEntityProfile profile ) throws DAOException {
        try {
            ByteArrayOutputStream buffer = new ByteArrayOutputStream();
            Writer myWriter = new BufferedWriter( new OutputStreamWriter( buffer, ApplicationConstants.UTF8 ) );
            profile.marshal( myWriter );
            myWriter.flush();
            return buffer;
        } catch ( Exception e ) {
